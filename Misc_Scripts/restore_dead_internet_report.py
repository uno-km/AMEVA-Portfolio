...
def build_original_report(filepath):
    doc = docx.Document()
    ...
    r_sub = p_sub.add_run("자율 에이전트 토론 시뮬레이션 및 여론 동역학 연구 보고서\n")
    ...
    p = doc.add_paragraph()
    p.add_run("1) 모델 크기별 컨텍스트 한계와 붕괴 현상\n").bold = True
    p.add_run(
    ...
    p.add_run("2) 경량 모델을 위한 컨텍스트 아키텍처 조율 및 제어\n").bold = True
    p.add_run(
    ...
    p.add_run("3) 정체성 유지를 위한 후처리 필터링\n").bold = True
    p.add_run(
    ...
    p.add_run("4) 심리 지표 반영 및 상위 에이전트 개입 반응성\n").bold = True
    p.add_run(
    ...
    p_dyn.add_run("1) 심리 상태 엔진 도입의 배경\n").bold = True
    p_dyn.add_run(
    ...
    p_dyn.add_run("3) 상호 대화에 따른 관계도 변화\n").bold = True
    p_dyn.add_run(
    ...
    p_dyn.add_run("4) 상대방 관계를 고려한 종합 분노 지수 산출\n").bold = True
    p_dyn.add_run(
    ...
    p_dyn.add_run("5) 언어 모델 연동 및 동적 발화 제어\n").bold = True
    p_dyn.add_run(
...
def main():
    ...
    paths = [
        "c:/ameva/AMEVA-Portfolio/finish/report_dead-internet-theatre.docx",
        "c:/ameva/AMEVA-Portfolio/report_dead-internet-theatre.docx"
    ]
    for p in paths:
        build_original_report(p)
